...
final class ContainerFingerprintTests: XCTestCase {
    ...
    func testHasSecondLeafReturnsFalseForSingleElement() {
        let tree = element(label: "Only")
        var found: AccessibilityElement?
        let result = tree.hasSecondLeaf(found: &found)

        XCTAssertFalse(result)
        XCTAssertEqual(found?.label, "Only")
    }

    func testHasSecondLeafReturnsTrueForTwoElements() {
        let tree = group(children: [
            element(label: "First", index: 0),
            element(label: "Second", index: 1),
        ])

        var found: AccessibilityElement?
        let result = tree.hasSecondLeaf(found: &found)

        XCTAssertTrue(result, "Should early-exit on second leaf")
    }

    func testHasSecondLeafReturnsFalseForEmptyContainer() {
        let tree = group(children: [])
        var found: AccessibilityElement?
        let result = tree.hasSecondLeaf(found: &found)
    ...
    func testHasSecondLeafPropagatesAcrossRoots() {
    ...
    func testHasSecondLeafSingleElementInDeepTree() {
        let tree = group(children: [
            group(children: [
                group(children: [
        ...
        XCTAssertFalse(result)
        XCTAssertEqual(found?.label, "Deep")
    }
}
#endif
