...
private extension HeistExecution.Effect {
    var observationScope: SemanticObservationScope? {
        guard case .beginObservation(_, let scope, _) = self else { return nil }
        return scope
    }

    var observationTimeout: Duration? {
        guard case .beginObservation(_, _, let deadline) = self else { return nil }
        return .seconds(deadline.timeoutSeconds)
    }
}
