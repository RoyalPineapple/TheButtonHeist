...
final class TheFenceTests: XCTestCase {
    ...
    func testDrainBackgroundAccessibilityTraceClearsAfterRead() async {
        ...
        XCTAssertEqual(first?.meaningfulEndpointDeltaProjection?.isScreenChanged, true)
        XCTAssertEqual(first?.meaningfulEndpointDeltaProjection?.elementCount, 7)
    ...
    func testDrainBackgroundAccessibilityTracePreservesArrivalOrder() async {
        ...
        let first = fence.drainBackgroundAccessibilityTrace()?.meaningfulEndpointDeltaProjection
        XCTAssertEqual(first?.kindRawValue, "elementsChanged")
        XCTAssertEqual(first?.elementCount, 2)

        let second = fence.drainBackgroundAccessibilityTrace()?.meaningfulEndpointDeltaProjection
    ...
    func testDrainBackgroundAccessibilityTracesReturnsAllQueuedTraces() async {
        ...
        let deltas = traces.compactMap(\.meaningfulEndpointDeltaProjection)
    ...
    func testBackgroundChangeIsStoredAsTraceAndDeltaIsDerivedAtTheEdge() async {
        ...
        let derived = drained?.meaningfulEndpointDeltaProjection
    ...
    func testBackgroundExpectationMismatchDoesNotConsumeDelta() async throws {
        ...
        let queued = fence.drainBackgroundAccessibilityTrace()?.meaningfulEndpointDeltaProjection
    ...
    func testWaitForChangeExpectationConsumesOnlyMatchingDelta() async throws {
        ...
        let remaining = fence.drainBackgroundAccessibilityTrace()?.meaningfulEndpointDeltaProjection
    ...
    func testBackgroundTraceQueueDropsOldestWhenCapacityExceeded() async {
        ...
        for expectedCount in 7...25 {
            XCTAssertEqual(fence.drainBackgroundAccessibilityTrace()?.meaningfulEndpointDeltaProjection?.elementCount, expectedCount)
        }
        XCTAssertNil(fence.drainBackgroundAccessibilityTrace())
    }
