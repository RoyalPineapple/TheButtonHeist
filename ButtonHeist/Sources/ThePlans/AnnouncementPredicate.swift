...
public struct AnnouncementPredicateExpr: Codable, Sendable, Equatable, Hashable {
    ...
    public init(_ text: String) {
        self.init(match: .exact(.literal(text)))
    }

    public func resolve(in environment: HeistExecutionEnvironment) throws -> AnnouncementPredicate {
        AnnouncementPredicate(match: try match?.resolve(in: environment))
    }

    public init(from decoder: Decoder) throws {
        try decoder.rejectUnknownKeys(allowed: CodingKeys.self, typeName: "announcement predicate expression")
        let container = try decoder.container(keyedBy: CodingKeys.self)
        self.init(match: try container.decodeIfPresent(StringMatch<StringExpr>.self, forKey: .match))
    }
...
extension AnnouncementPredicateExpr: CustomStringConvertible {
    public var description: String {
        guard let match else { return "announcement" }
        return ScoreDescription.call("announcement", [match.description])
    }
