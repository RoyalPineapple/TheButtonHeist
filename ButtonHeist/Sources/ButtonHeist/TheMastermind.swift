...
public final class TheMastermind {
    ...
    public static let supportedCommands = MastermindCommandCatalog.all
    ...
    public func execute(request: [String: Any]) async throws -> MastermindResponse {
        guard let command = request["command"] as? String else {
            throw MastermindError.invalidRequest("Invalid JSON or missing 'command' field")
        }
    ...
    private func connect() async throws {
        ...
        while client.discoveredDevices.first(matching: filter) == nil {
            if DispatchTime.now().uptimeNanoseconds - discoveryStart > discoveryTimeout {
                if let filter {
                    throw MastermindError.noMatchingDevice(
                        filter: filter,
                        available: client.discoveredDevices.map(\.name)
                    )
                }
                throw MastermindError.noDeviceFound
            }
        ...
        client.onAuthFailed = { reason in
            connectionError = MastermindError.authFailed(reason)
        }
        client.onSessionLocked = { payload in
            connectionError = MastermindError.sessionLocked(payload.message)
        }
        ...
        while !connected && connectionError == nil {
            if DispatchTime.now().uptimeNanoseconds - connectionStart > connectionTimeout {
                throw MastermindError.connectionTimeout
            }
            try await Task.sleep(nanoseconds: 100_000_000)
        }

        if let connectionError {
            if let mastermindError = connectionError as? MastermindError {
                throw mastermindError
            }
            throw MastermindError.connectionFailed(connectionError.localizedDescription)
        }
    ...
    private func dispatch(command: String, args: [String: Any]) async throws -> MastermindResponse {
        switch command {
        case "status":
            return .status(
        ...
        case "get_interface":
            return .interface(try await requestInterface())

        case "get_screen":
            guard client.connectionState == .connected else { throw MastermindError.notConnected }
            ...
            do {
                ...
                throw MastermindError.actionTimeout
            }

        case "wait_for_idle":
            return try await sendAction(
                .waitForIdle(WaitForIdleTarget(timeout: doubleArg(args, "timeout")))
            )
        ...
        case "swipe":
            ...
            return try await sendAction(
                .touchSwipe(
                    SwipeTarget(
                        elementTarget: elementTarget(args),
                        startX: doubleArg(args, "startX"),
                        startY: doubleArg(args, "startY"),
                        endX: doubleArg(args, "endX"),
                        endY: doubleArg(args, "endY"),
                        direction: direction,
        ...
        case "pinch":
            ...
            return try await sendAction(
                .touchPinch(
                    PinchTarget(
                        elementTarget: elementTarget(args),
                        centerX: doubleArg(args, "centerX"),
                        centerY: doubleArg(args, "centerY"),
                        scale: scale,
        ...
        case "draw_bezier":
            ...
            for segment in segmentsArray {
                guard
                    let cp1X = numberArg(segment["cp1X"]),
                    let cp1Y = numberArg(segment["cp1Y"]),
                    let cp2X = numberArg(segment["cp2X"]),
                    let cp2Y = numberArg(segment["cp2Y"]),
                    let endX = numberArg(segment["endX"]),
                    let endY = numberArg(segment["endY"])
                else {
                    return .error("Each segment needs cp1X, cp1Y, cp2X, cp2Y, endX, endY")
                }
                segments.append(
                    BezierSegment(cp1X: cp1X, cp1Y: cp1Y, cp2X: cp2X, cp2Y: cp2Y, endX: endX, endY: endY)
                )
            }
        ...
        case "perform_custom_action":
            ...
            return try await sendAction(
                .performCustomAction(CustomActionTarget(elementTarget: target, actionName: actionName))
            )
    ...
    private func sendAction(_ message: ClientMessage) async throws -> MastermindResponse {
        guard client.connectionState == .connected else { throw MastermindError.notConnected }
        client.send(message)
        do {
