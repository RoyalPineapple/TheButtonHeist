...
final class TheSafecracker {
    ...
    func tap(at point: CGPoint) async -> Bool {
        ...
        guard await cancellableSleep(for: Self.gestureYieldDelay) else { return false }
        return touchUp()
    }
    ...
    func typeText(_ text: String, interKeyDelay: UInt64 = TheSafecracker.defaultInterKeyDelay) async -> Bool {
        ...
        for char in text {
            keyboard.type(char)
            guard await cancellableSleep(nanoseconds: interKeyDelay) else { break }
        }
        return true
    }
    ...
    func clearText() async -> Bool {
        ...
        guard await cancellableSleep(for: Self.selectionSettleDelay) else { return false }
