...
**Inactivity monitor** — wakes every 1s, checks `Date().timeIntervalSince(lastActivityTime)` against `inactivityTimeout`. Activity is bumped by `noteActivity()` (incoming commands) and `noteScreenChange()` (hierarchy broadcasts).
