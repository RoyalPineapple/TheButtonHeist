...
5. **`TheStash+Matching.swift`** — `matchScreenElements(matcher, limit:)` is hierarchy-first, registry-fallback. First walks `currentHierarchy` via early-exit `compactMap(first:)`, maps hits through `reverseIndex` to heistIds. If no hierarchy hits, falls back to iterating the full `registry.elements` (catches off-screen explored elements). `AccessibilityElement.matches(_:)` uses `localizedCaseInsensitiveContains` for strings; validates unknown traits against `knownTraitNames` before bitmask comparison (fail-safe).
