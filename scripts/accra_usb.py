#!/usr/bin/env python3
"""
Accra USB Connection Module

Provides reliable connection to AccraHost on iOS devices over USB
using the CoreDevice IPv6 tunnel.

Usage:
    from accra_usb import AccraUSBConnection
...
class AccraUSBError(Exception):
    """Base exception for Accra USB connection errors."""
    pass


class DeviceNotFoundError(AccraUSBError):
    """Raised when the specified device is not found or not connected."""
    pass


class ConnectionError(AccraUSBError):
    """Raised when connection to AccraHost fails."""
    pass


class AccraUSBConnection:
    """
    Manages a connection to AccraHost on an iOS device over USB.
    ...
    def __enter__(self) -> "AccraUSBConnection":
        self.connect()
        return self
    ...
    def connect(self) -> None:
        """
        Establish connection to AccraHost.
        ...
        if self.auto_launch:
            self._launch_app()
            time.sleep(3)  # Wait for app to start and AccraHost to initialize
    ...
    def _find_port(self) -> int:
        ...
        raise ConnectionError("Could not find AccraHost port")

    def _test_port(self, port: int) -> bool:
        """Test if a port has AccraHost."""
        try:
...
def quick_connect(device_name: Optional[str] = None) -> AccraUSBConnection:
    """
    ...
    Example:
        ...
        conn.close()
    """
    conn = AccraUSBConnection(device_name=device_name)
    conn.connect()
    return conn
...
if __name__ == "__main__":
    # Demo usage
    print("Connecting to AccraHost over USB...")
    with AccraUSBConnection() as conn:
