...
ALLOWED_TARGET_DEPENDENCY_DIFFERENCES = {
    # Root Package.swift publishes TheInsideJob as a product containing both
    # TheInsideJob and ThePlant targets. The nested local package also models
    # the ObjC loader's link-time relationship explicitly for local builds.
    "ThePlant": {
        "root": (),
        "buttonheist": ("target:TheInsideJob",),
    },
}
...
def check_targets(root: dict[str, Any], buttonheist: dict[str, Any], failures: list[str]) -> None:
    ...
    for target_name in CONTRACT_TARGETS:
        ...
        allowed_dependency_difference = ALLOWED_TARGET_DEPENDENCY_DIFFERENCES.get(target_name)
        if allowed_dependency_difference:
            root_summary = dict(root_summary)
            buttonheist_summary = dict(buttonheist_summary)
            allowed_root = allowed_dependency_difference["root"]
            allowed_buttonheist = allowed_dependency_difference["buttonheist"]
            if (
                root_summary["dependencies"] == allowed_root
                and buttonheist_summary["dependencies"] == allowed_buttonheist
            ):
                root_summary["dependencies"] = ("<allowed difference>",)
                buttonheist_summary["dependencies"] = ("<allowed difference>",)
...
def main() -> None:
    ...
    if failures:
        ...
        print(
            ...
            "in ButtonHeist/Package.swift; ThePlant's explicit TheInsideJob edge is local-only.",
            file=sys.stderr,
        )
        raise SystemExit(1)
    ...
    print(
        ...
        "ThePlant -> TheInsideJob is explicit only in ButtonHeist/Package.swift."
    )
