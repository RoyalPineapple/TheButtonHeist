#!/usr/bin/env bash
# Find a matching passed/failed receipt pair and run heist-doctor.
...
usage() {
    cat <<'EOF'
Usage: scripts/heist-doctor-from-receipts.sh [options]
...
The script matches receipts by their parent heist-name/fingerprint directory.
It selects the newest failed receipt that has a matching passed receipt, then
...
if [[ ! -d "$LAST_PASS_DIR" ]]; then
    echo "Error: last-pass receipt directory not found: $LAST_PASS_DIR" >&2
    exit 1
fi
if [[ ! -d "$NEW_FAIL_DIR" ]]; then
    echo "Error: new-fail receipt directory not found: $NEW_FAIL_DIR" >&2
    exit 1
fi
...
def receipts(root: pathlib.Path, status: str) -> list[pathlib.Path]:
...
for receipt in receipts(last_pass_dir, "passed"):
    passes_by_key.setdefault(receipt.parent.name, []).append(receipt)

for failed in receipts(new_fail_dir, "failed"):
...
print(
    "Error: no doctor-ready receipt pair found. "
    "Need a failed receipt and a passed receipt with the same heist fingerprint directory.",
    file=sys.stderr,
)
raise SystemExit(1)
PY
...
LAST_PASS_RECEIPT="$(printf '%s\n' "$PAIR_OUTPUT" | sed -n '1p')"
NEW_FAIL_RECEIPT="$(printf '%s\n' "$PAIR_OUTPUT" | sed -n '2p')"
FINGERPRINT_DIR="$(printf '%s\n' "$PAIR_OUTPUT" | sed -n '3p')"

echo "Selected doctor receipt pair:"
echo "  fingerprint: $FINGERPRINT_DIR"
echo "  last pass:   $LAST_PASS_RECEIPT"
echo "  new fail:    $NEW_FAIL_RECEIPT"
echo

ARGS=(
    --last-pass "$LAST_PASS_RECEIPT"
    --new-fail "$NEW_FAIL_RECEIPT"
    --format "$FORMAT"
)
if [[ -n "$STEP_PATH" ]]; then
