...
**Sessions contributing**: 5 (fuzzsession-2026-02-18-1751-map-screens.md, fuzzsession-2026-02-19-1127-explore-todo-list.md, fuzzsession-2026-02-19-1234-explore-revisit.md, fuzzsession-2026-02-19-1239-explore-settings.md, fuzzsession-2026-02-19-1346-fuzz-state-exploration.md)

## Screens

| # | Name | Fingerprint | Elements | Entry Points |
|---|------|-------------|----------|--------------|
| 1 | Main Menu | [Controls Demo, Todo List, Touch Canvas, Settings, ButtonHeist Test App heading] | 5 | App launch root |
| 14 | Todo List | [buttonheist.todo.newItemField, addButton, itemCount, emptyLabel, filter All/Active/Completed] | 11+ | Main Menu → "Todo List" |
...
| 16 | Date Picker calendar | [Month adjustable, day buttons (Feb 1..28), PopoverDismissRegion] | 30 | Toggles & Pickers → "Date Picker" button |
...
| Toggles & Pickers | activate "Date Picker" button (order 6) | Date Picker calendar popover | activate PopoverDismissRegion | YES |
| Toggles & Pickers | activate/tap colorPicker | Color picker sheet | tap (50,100) [UNRELIABLE — triggers back nav] | UNRELIABLE |
...
| Date Picker calendar | activate PopoverDismissRegion | Toggles & Pickers | (popover dismiss) |
...
### Explored sub-interactions (session 2026-02-19)
- Toggles & Pickers: menuPicker opens [Daily/Weekly/Monthly] menu ✓; Date Picker opens full calendar popover ✓; colorPicker opens visual sheet but 0 a11y elements (F-4)
- Buttons & Actions: optionsMenu opens [Option A/Option B/Delete] context menu ✓; customActionsItem has Share+Favorite custom actions ✓
- Display: learnMoreLink opens Safari externally (noChange in app) ✓
- Disclosure & Grouping: advancedGroup toggles expand/collapse ✓; revealed toggles (Enable notifications, Dark mode) are inert (F-6); all 3 share same identifier (F-5)

### New findings (session 2026-02-19)
- F-1: Todo List — items not persisted across navigation (data lost on back/return)
- F-2: Todo List — empty state "No all todos" grammatically broken for "All" filter
- F-3: Todo List — "Clear Completed" visible when showCompleted=OFF; Completed filter always empty
- F-4: Toggles & Pickers — colorPicker exposes 0 a11y elements, dismiss is unreliable/causes back nav
- F-5: Disclosure & Grouping — 3 elements share identifier "buttonheist.disclosure.advancedGroup"
- F-6: Disclosure & Grouping — disclosed toggles (Enable notifications, Dark mode) are completely inert

### Settings cross-screen effects (verified 2026-02-19)
- Dark mode: applied app-wide immediately and persists
- Accent color: applied app-wide immediately and persists
- Large text size: causes nav bar to switch from large-title to inline-title style; section headings grow
- Show Completed Todos: directly affects Todo List filter/empty state behavior
- All settings persist across navigation (navigate away and return)
