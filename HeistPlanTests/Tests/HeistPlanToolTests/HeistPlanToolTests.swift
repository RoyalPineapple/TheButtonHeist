...
struct HeistPlanToolTests {
    ...
    func `compile round trips Swift DSL through JSON and canonical Swift`() throws {
        ...
        #expect(result.exitCode == 0, "\(result.stderr)")
        let data = try Data(contentsOf: outputURL.appendingPathComponent("plan.json"))
        let plan = try HeistArtifactCodec.readPlan(from: outputURL)
        ...
        #expect(try Data(contentsOf: roundTrippedOutputURL.appendingPathComponent("plan.json")) == data)
    }
