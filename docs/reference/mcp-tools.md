...
| `describe_heist` | `heistRuntime` | Describe one root entry or reusable heist from a runtime-validated plan. The `heist` parameter selects the entry/capability name; the plan can be supplied as canonical ButtonHeist source via `plan`, as structured HeistPlan fields (version, name, parameter, definitions, body), or loaded from a `path` to a .heist package artifact. |
