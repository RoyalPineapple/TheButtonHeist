...
| **TheHandoff** | Passes `token` to DeviceConnection. Stores approved tokens via `onAuthApproved` callback. Tracks `connectionState` (including `.failed` on auth failure or session lock). |
