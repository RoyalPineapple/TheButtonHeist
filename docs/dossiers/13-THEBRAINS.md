...
`Navigation` and `Actions` are *internal components of TheBrains*, not crew members in their own right — neutral noun-style names. Both are owned by TheBrains (`let navigation: Navigation`, `let actions: Actions`) and share the same TheStash / TheSafecracker / TheTripwire references. Actions holds a reference to Navigation because targeted text entry calls `navigation.ensureOnScreen(for:)` before tapping a field.
...
graph TD
    ...
    Dispatch --> Navigation
    Dispatch --> Core
    Actions -->|"navigation.ensureOnScreen"| Navigation
